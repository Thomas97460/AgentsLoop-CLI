...
from agentic_workflows.domain.models import WorkflowState
from agentic_workflows.runtime.agent_runner import AgentRunSpec, run_agent
from agentic_workflows.runtime.templates import (
    extract_section,
...
def build_ci_summary(state: WorkflowState) -> str:
    """Return the CTO prompt CI validation block."""
    ci = state.validation.get("ci")
    if not isinstance(ci, dict) or not ci:
        return "_none_"
    return (
        f"status: {ci.get('status') or ''}\n"
        f"exit_code: {ci.get('exit_code')}\n"
        f"branch: {ci.get('developer_branch') or ''}\n"
        f"command: {ci.get('command') or ''}\n"
        f"output:\n{ci.get('output') or ''}"
    )
...
def build_prompt(state: WorkflowState, prompts_dir: Path) -> str:
    ...
    return render_template(
        prompts_dir / "cto_prompt.md",
        {
            ...
            "ci_quiet_validation": build_ci_summary(state),
        },
    )
