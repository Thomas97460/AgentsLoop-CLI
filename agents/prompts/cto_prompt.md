...
- Each agent runs in an isolated repository clone under `~/tmp/algosia-agents/.../repo`, so git push is required to persist developer work.
...
- NEVER return `done` when CI Quiet Validation status is not `success`; return `continue` and delegate a fix to the developer.
...
- CI Quiet Validation status is `success`
