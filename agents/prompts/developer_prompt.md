...
- Your execution workspace is an isolated repository clone under `~/tmp/algosia-agents/.../repo`, so git push is required to persist your work.
...
- If you modify files, you must run `task ci-quiet`, commit, and push on `working_branch` before reporting `Task completed`.
...
- run `task ci-quiet` before your final report when the repository was modified
