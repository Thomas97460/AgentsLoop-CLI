...
from agentsloop.domain.models import NodeStatus, WorkflowState
from agentsloop.runtime.git_runtime import clone_for_agent
from agentsloop.storage.json_store import RunStore

VALIDATION_LOG_LINES = 160


def run_validation(state: WorkflowState, env: dict[str, str], store: RunStore) -> None:
    """Clone the developer branch and run the configured validation command."""
    ...
    with (
        ...
        process = subprocess.Popen(
            ["bash", "-lc", state.config.validation_command],
            cwd=repo_path,
            env=env,
            text=True,
            stdout=stdout,
        ...
        process.wait()
    output = _first_lines((node_run.stdout_path, node_run.stderr_path), VALIDATION_LOG_LINES)
    status: NodeStatus = "success" if process.returncode == 0 else "error"
    validation_result = {
        "status": status,
        "exit_code": process.returncode,
        "developer_branch": state.developer_branch,
        "command": state.config.validation_command,
    ...
    store.write_node_result(state, node_run, validation_result)
